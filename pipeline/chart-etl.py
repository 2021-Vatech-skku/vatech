...
df = df.withColumn("test", from_json(df.fullDocument, schema))
df.select("test").show(10, truncate=False)
df.printSchema()

# sc.set("spark.hadoop.fs.s3a.endpoint", "http://minio.it.vsmart00.com")
df.write.format("delta").conf("spark.hadoop.fs.s3a.endpoint", "http://minio.it.vsmart00.com").save("s3a://songhyun/chart")